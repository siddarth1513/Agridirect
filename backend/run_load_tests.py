...
def main():
    print("====================================================")
    print("Starting Django Endpoint Load & Performance Test Suite")
    print("====================================================")
    
    # Discover and load tests from accounts app specifically for load_tests.py
    suite = defaultTestLoader.discover('accounts', pattern='load_tests.py')
    ...
    print("\nWriting load test results to Excel sheet...")
    df = pd.DataFrame(result.results_list)
    output_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'load_test_results.xlsx')
    
    # Style and write using Excel
    with pd.ExcelWriter(output_path, engine='openpyxl') as writer:
        df.to_excel(writer, index=False, sheet_name='Load Tests')
        
    print(f"Results successfully saved to: {output_path}")
    print(f"Total Load Tests Executed: {len(df)}")
