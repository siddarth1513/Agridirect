// appium/scripts/generate_reports.js
// ============================================================
// Generates Excel (7-sheet) + HTML dashboard from
// execution-results.json – identical structure to Selenium suite
// ============================================================
'use strict';
...
if (!fs.existsSync(JSON_FILE)) {
  console.error('ERROR: execution-results.json not found. Run run_appium_tests.js first.');
  process.exit(1);
}
...
const COLS = [
  ...
  { header: 'Screenshot',          key: 'screenshot',    width: 38 },
  { header: 'Device Log',          key: 'deviceLog',     width: 30 },
];
  ...
  rows.forEach((r, idx) => {
    ...
    row.eachCell(cell => {
      ...
      if (cell._column.key === 'status') {
        const v = cell.value;
        cell.font = { bold: true, color: { argb: v==='PASSED'?'155724':v==='FAILED'?'721C24':'856404' } };
        cell.fill = { type:'pattern', pattern:'solid',
          fgColor: { argb: v==='PASSED'?CLR.passGreen:v==='FAILED'?CLR.failRed:CLR.yellow } };
      }
    });
    row.height = 18;
  });
...
async function generateExcel() {
  ...
  wb.creator    = 'Farmers App QA – Appium Suite';
  wb.created    = new Date();
  ...
  const modCounts = {};
  results.forEach(r => {
    if (!modCounts[r.module]) modCounts[r.module] = { passed: 0, failed: 0, total: 0 };
    modCounts[r.module].total++;
    if (r.status === 'PASSED') modCounts[r.module].passed++;
    else modCounts[r.module].failed++;
  });

  const avgDur = (results.reduce((s,r)=>s+r.executionTime,0)/results.length).toFixed(2);
  ...
  addDataRows(shMet, [
    ...
    { metric: 'Application Name',       value: 'Farmers App – AgriDirect (Android)' },
    { metric: 'App Package',            value: 'com.example.farmers_app' },
    { metric: 'Device',                 value: 'Android Emulator (API 33)' },
    { metric: 'Android Version',        value: '13 (API Level 33)' },
    { metric: 'Automation Engine',      value: 'UiAutomator2 via Appium' },
    { metric: 'Framework',              value: 'Node.js Appium Suite' },
    ...
    { metric: 'Avg Test Duration (ms)', value: avgDur },
    { metric: 'Min Test Duration (ms)', value: minDur },
    { metric: 'Max Test Duration (ms)', value: maxDur },
    { metric: '--- Module Breakdown ---', value: '' },
    ...Object.entries(modCounts).map(([m, c]) => ({
      metric: `  ${m}`,
      value: `Total: ${c.total} | Passed: ${c.passed} | Failed: ${c.failed}`,
    })),
  ], CLR.skyBlue);
  ...
  styleHeader(shDef);
  if (failed.length === 0) {
    const row = shDef.addRow({ testId:'–', module:'–', screen:'–',
      testName:'No defects found – all 300 tests passed! ✅', error:'–', screenshot:'–' });
    ...
    addDataRows(shDef, failed.map(r => ({
      testId: r.testId, module: r.module, screen: r.screen,
      testName: r.testName, error: r.error || '–', screenshot: r.screenshot,
    })), CLR.failRed);
  }
  ...
  styleHeader(shPR);
  const prRows = Object.entries(modCounts).map(([m, c]) => ({
    module: m, total: c.total, passed: c.passed, failed: c.failed,
    passRate: `${((c.passed / c.total) * 100).toFixed(1)}%`,
  }));
  prRows.push({ module:'OVERALL', total:summary.total, passed:summary.passed,
    failed:summary.failed, passRate:`${passRate}%` });
  addDataRows(shPR, prRows, CLR.skyBlue);

  const xlsxPath = path.join(REPORTS_DIR, 'Appium_Test_Report.xlsx');
...
function generateHtml() {
  const modCounts = {};
  results.forEach(r => {
    if (!modCounts[r.module]) modCounts[r.module] = 0;
    modCounts[r.module]++;
  });

  const labels = JSON.stringify(Object.keys(modCounts));
  const data   = JSON.stringify(Object.values(modCounts));
...
<title>Farmers App – Appium Android E2E Execution Report</title>
...
<style>
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  .device-badge{display:inline-block;background:rgba(255,255,255,.2);border-radius:20px;
    padding:.3rem 1rem;margin:.5rem .25rem;font-size:.85rem;font-weight:600}
  ...
  .num-total{color:#0a3d62}.num-pass{color:#28a745}.num-fail{color:#dc3545}
  .num-skip{color:#ffc107}.num-rate{color:#17a2b8}
  ...
  table{width:100%;border-collapse:collapse;background:#fff;border-radius:10px;
    overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,.07);font-size:.82rem}
  thead th{background:#0a3d62;color:#fff;padding:.75rem .5rem;text-align:left;font-weight:600}
  tbody tr:nth-child(even){background:#f8f9ff}
  tbody td{padding:.55rem .5rem;border-bottom:1px solid #eee}
  ...
  code{background:#e9ecef;padding:.15rem .4rem;border-radius:4px;font-size:.78rem}
  .chart-wrap{background:rgba(255,255,255,.75);backdrop-filter:blur(10px);
    border-radius:14px;padding:1.5rem;box-shadow:0 4px 16px rgba(0,0,0,.08);max-width:640px}
  footer{text-align:center;padding:2rem;color:#666;font-size:.8rem}
</style>
</head>
<body>
<div class="header">
  <h1>📱 Farmers App – Appium Android E2E Test Report</h1>
  <p>
    <span class="device-badge">📱 Android Emulator (API 33)</span>
    <span class="device-badge">⚙️ UiAutomator2</span>
    <span class="device-badge">📦 com.example.farmers_app</span>
  </p>
  <p>Generated: <strong>${runDate}</strong> &nbsp;|&nbsp; Duration: <strong>${duration}s</strong></p>
</div>
...
<div class="section">
  <h2>All Test Cases (300) – Android Mobile</h2>
  <table>
    <thead>
      <tr>
        <th>#</th><th>Test ID</th><th>Module</th><th>Screen</th>
        <th>Test Name</th><th>Priority</th><th>Status</th><th>Time</th>
      </tr>
    </thead>
    <tbody>${rows}</tbody>
  </table>
...
<footer>Farmers App QA – Appium Android Report &copy; ${new Date().getFullYear()}</footer>
...
new Chart(document.getElementById('moduleChart').getContext('2d'),{
  ...
  data:{
    ...
    datasets:[{
      label:'Test Cases',
      data:${data},
      backgroundColor:['#0a3d62','#1a5b8c','#28a745','#17a2b8',
        '#ffc107','#fd7e14','#6f42c1','#e83e8c','#20c997','#dc3545'],
      borderRadius:6
    }]
  },
  options:{
  ...
  const htmlPath = path.join(REPORTS_DIR, 'appium-execution-report.html');
...
(async () => {
  ...
  console.log('\n  Generating Appium reports …\n');
  await generateExcel();
  generateHtml();
  console.log('\n  ✅ All Appium reports generated successfully!\n');
})();
